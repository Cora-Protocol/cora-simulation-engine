...
from libs.curve_gen.domain_transformation import transform_pdf_using_optimize
from libs.curve_gen.convolution.builder import (
    ConvolutionConfig,
    ConvolutionConfigBuilder,
)
from libs.curve_gen.convolution.helpers import convolve_self_n
...
class LogDomainConvolver:
    ...
    def run_convolution(
        self, expiration_days=(), transform=transform_pdf_using_optimize
    ):
        """
    ...
    def _create_log_pdfs(self):
        ...
        None
        """
        self.log_pdf_list = convolve_self_n(
            self.config.dist, self.config.log_x, self.config.num_times_to_conv
        )
    ...
    def _create_price_pdfs(self, expiration_days=()):
        ...
        self.price_pdf_list = [
            self.transform_pdf_func(self.config.log_x, log_pdf, self.config.x, tol=1e-6)

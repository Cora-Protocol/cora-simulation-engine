...
def bind_pdf_params(
    dist: rv_continuous, loc: float, scale: float, skew: float, nu: float
):
    """
    ...
    return partial(
        lambda l, sc, sk, n, x: dist.pdf(x, sk, n, loc=l, scale=sc),
        loc,
        scale,
        skew,
        nu,
...
def convolve_self_n(dist: partial, x, n: int):
    ...
    """
    # NOTE: 3 This is where the day by day convolution is done

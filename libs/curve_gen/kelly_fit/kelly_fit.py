...
from libs.curve_gen.kelly_fit.builder import FitConfig, FitConfigBuilder
...
def _fit_poly(
    ...
    """
    fit_params, fit_cov = curve_fit(
        lambda t, a, b, c, d: a * t**3 + b * t**2 + c * t + d,
        bet_fractions,
        premiums,
        maxfev=maxfev,
        bounds=(lower_bounds, upper_bounds),
...
fit_function_table = {"EXP": _fit_exp, "POLY": _fit_poly, "COSH": _fit_cosh}
...
class KellyFit:
    ...
    def fit_kelly_curve(
        ...
        params : List[float]
            The list of parameters of the function fit to the Kelly curve
        """
        range_to_use = np.where(
            (np.asarray(premiums) > 1e-10) | (np.asarray(premiums) < -1e-10)
        )[0][-1]

        return self.fit_func(
            bet_fractions[: range_to_use + 1],
            premiums[: range_to_use + 1],
            maxfev=maxfev,

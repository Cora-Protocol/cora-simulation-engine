...
from libs.curve_gen.training.fit.helpers import calc_log_returns
from libs.curve_gen.training.distributions.skewed_students_t import skewed_t
from libs.curve_gen.training.builder import TrainingConfig, TrainingConfigBuilder
...
def get_training_dates(csv_df):
    ...
    price_history_dates = np.asarray(
        [
...
def _extract_historical_price_path(
    csv_df: pd.DataFrame,
    ...
    start_date, starting_index = parse_date_from_str(
        start_date_str,
        price_history_dates,
        full_price_path,
        limit_str="min",
    ...
    end_date, ending_index = parse_date_from_str(
        end_date_str,
        price_history_dates,
        full_price_path,
        limit_str="max",
...
def _filter_convolution_group(csv_df: pd.DataFrame, row):
    ...
    return (
        csv_df[
            (csv_df["Asset"] == row.Asset)
            & (csv_df["TrainingLabel"] == row.TrainingLabel)
            & (csv_df["TrainingStart"] == row.TrainingStart)
            & (csv_df["TrainingEnd"] == row.TrainingEnd)
        ]
        .drop_duplicates()
...
def _fit_params_from_prices(
    price_path, *args, dist=skewed_t, calc_returns=calc_log_returns
):
    """
...
class Trainer:
    ...
    def _create_convolution_group(
        self, row, training_dates, *args, dist=skewed_t, calc_returns=calc_log_returns
    ):
        """
        For all rows in the input CSV which have the same Asset, TrainingLabel, TrainingStart,
        and TrainingEnd, they will have the same results when the convolution is performed during
        ...
        # NOTE: 1. This is where the distribution is being fit to the training data
        dist_params = _fit_params_from_prices(
            price_path, *args, dist=dist, calc_returns=calc_returns
        )
    ...
    def train(self, *args, dist=skewed_t, calc_returns=calc_log_returns):
        ...
        conv_dfs = [
            self._create_convolution_group(
                row, training_dates, *args, dist=dist, calc_returns=calc_returns
            )

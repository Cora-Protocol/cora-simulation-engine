...
def _error_file_not_found(filename: str):
    ...
    raise FileNotFoundError(
        "Check configuration. Could not find File: {}".format(filename)
    )
...
def _check_input_format(csv_df: pd.DataFrame):
    ...
    # print('Asset' == csv_df.columns[0])
    # print('TrainingLabel' == csv_df.columns[1])
    # print('TrainingStart' == csv_df.columns[2])
    # print('TrainingEnd' == csv_df.columns[3])
    # print('StrikePct' == csv_df.columns[4])
    # print('Expiration' == csv_df.columns[5])
    # print('CurrentPrice' == csv_df.columns[6])

    return (
        ("Asset" == csv_df.columns[0])
        and ("TrainingLabel" == csv_df.columns[1])
        and ("TrainingStart" == csv_df.columns[2])
        and ("TrainingEnd" == csv_df.columns[3])
        and ("StrikePct" == csv_df.columns[4])
        and ("Expiration" == csv_df.columns[5])
        and ("CurrentPrice" == csv_df.columns[6])
    )
...
def _verify_format(
    filename: str,
    csv_df: pd.DataFrame,
    check_format=_check_input_format,
...
def _load_csv_file(
    filename: str,
    sep=",",
    check_file_exists=isfile,
    on_file_error=_error_file_not_found,
    on_format_error=_error_bad_csv_format,
):
    """
...
class TrainingConfigBuilder:
    ...
    def build_config(
        ...
        check_input_format=_check_input_format,
        on_format_error=_error_bad_csv_format,
    ):
        """
        ...
        training_csv_df = _load_csv_file(
            self.training_history_filename,
            check_file_exists=check_file_exists,
            on_file_error=on_file_error,
            on_format_error=on_format_error,
        )
        input_csv_df = _load_csv_file(
        ...
        _verify_format(
            self.training_history_filename,
            training_csv_df,
            check_format=check_training_format,
            on_format_error=on_format_error,
        )
        _verify_format(
            self.input_csv_filename,
            input_csv_df,
            check_format=check_input_format,
            on_format_error=on_format_error,
        )
        ...
        # print('input_asset_list ', input_asset_list)
        # print('training_asset_list ', training_asset_list)
    ...
    def from_config(
        self, configs: List[CurveConfig], price_history: List[Tuple[datetime, float]]
    ) -> Self:
        input_asset_list = list(set([c.asset for c in configs]))
        assert len(input_asset_list) == 1, "All configs must have the same asset"
        asset = input_asset_list[0]

        # Create the training df
        date_strs = [d.strftime("%d/%m/%Y") for d, p in price_history]
        prices = [p for d, p in price_history]
        training_df = pd.DataFrame({"Master calendar": date_strs, asset: prices})
        ...
        input_df = pd.DataFrame(
            [
                {
                    "Asset": asset,
                    "TrainingLabel": config.training_label,
                    "TrainingStart": config.training_start.strftime("%d/%m/%Y"),
                    "TrainingEnd": config.training_end.strftime("%d/%m/%Y"),
                    "StrikePct": config.strike_percent,
        ...
        return TrainingConfig(training_df, input_df)

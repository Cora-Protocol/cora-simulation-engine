...
from libs.curve_gen.builder import GeneratorConfigBuilder, GeneratorConfig
from libs.curve_gen.kelly import probability_from_density, kelly_formula_derivative
from libs.curve_gen.payoff.builder import PayoffConfig
from libs.curve_gen.utils import (
    training_output_to_csv,
    training_output_to_convolution_config,
    training_output_to_payoff_config,
    add_pdf_csv_columns,
    add_curve_csv_rows,
)

from libs.curve_gen.training.distributions.skewed_students_t import skewed_t
from libs.curve_gen.training.train import configure_training, train
from libs.curve_gen.convolution.convolution import (
    configure_convolution,
    run_convolution,
    get_pdf_arrays,
)
from libs.curve_gen.constraints.bounds import (
    configure_bounds,
    get_lower_bound,
    get_upper_bound,
)
from libs.curve_gen.kelly_fit.kelly_fit import configure_fit, fit_kelly_curve
from libs.curve_gen.payoff.payoff import configure_payoff, get_payoff_odds
...
def _perform_training(*args, dist=skewed_t, payoff_dict=None):
    ...
    payoff_configs = training_output_to_payoff_config(
        conv_dfs, conv_configs, payoff_dict=payoff_dict
    )
...
def _generate_premiums(
    bet_fractions: np.ndarray, prob_bins: np.ndarray, bounds_dict=None
):
    """
    ...
    for bf_i, bf in enumerate(bet_fractions):
        ...
        try:
            premium = brentq(
                _kelly_derivative, lower_bound, upper_bound, args=(bf, prob_bins)
            )
...
def _generate_curves(
    payoff_cfg, expiration_days, bet_fractions=np.linspace(0.0, 0.9999, 50)
):
    """
    ...
    payoffs: List[PayoffConfig] = list(dict.fromkeys(payoff_cfg))
    ...
    last_strike_map = {
        ...
        for current_strike, last_strike in zip(strikes, shifted_strikes)
    }
    next_last_strike_map = {
        current_strike: next_last_strike
        for current_strike, next_last_strike in zip(strikes, shifted_strikes_two)
    }

    shfted_exps = pd.Series(uexps).shift(-1).tolist()
    next_exp_map = {
        current_exp: next_exp for current_exp, next_exp in zip(uexps, shfted_exps)
    }
    ...
    for day_index, (day_count, strike_payoff_cfg) in enumerate(tuple_list):
        ...
        # NOTE 5: Once we have the prop bins, this is where the premium is calculated using Kelly
        opt_premiums = _generate_premiums(
            bet_fractions, prob_bins, bounds_dict=bounds_dict
        )

        # Fit A, B, C, D parameters to the curve
        # NOTE 6: From that array of optimals, we fit it to a COSH-based curve
        fit_params = _fit_curve(
            bet_fractions,
            opt_premiums,
    ...
    outs = [
        outputs[exp][strike.option_legs[0]["strike"]]
        for exp in uexps
        for strike in payoffs
    ]
...
class Generator:
    ...
    def generate_curves(
        ...
        conv_dfs, conv_configs, payoff_configs = _perform_training(
            *initial_guess, dist=dist, payoff_dict=payoff_dict
        )
        # Run each convolution batch and save the output
        curves_df_rows = []
        column_data = []
        column_names = []
        for index, (df, conv_cfg, payoff_cfg) in enumerate(
            zip(conv_dfs, conv_configs, payoff_configs)
        ):
            ...
            column_names, column_data = add_pdf_csv_columns(
                ...
                column_data,
                df["Asset"].values[0],
                df["TrainingLabel"].values[0],
                get_pdfs_fcn=_conv_get,
            )

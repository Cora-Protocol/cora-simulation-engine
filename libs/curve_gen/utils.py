...
from libs.curve_gen.payoff.helpers import black_scholes_payoff
from libs.curve_gen.payoff.builder import PayoffConfigBuilder
from libs.curve_gen.convolution.builder import (
    ConvolutionConfig,
    ConvolutionConfigBuilder,
)
from libs.curve_gen.convolution.convolution import get_pdf_arrays
from libs.curve_gen.training.distributions.skewed_students_t import skewed_t
from libs.curve_gen.training.builder import TrainingConfigBuilder, CurveConfig
from libs.curve_gen.kelly_fit.builder import FitConfigBuilder
from libs.curve_gen.builder import GeneratorConfig
from libs.curve_gen.constraints.builder import ConstraintsConfig
from typing import List, Tuple


def make_payoff_dict(
    call_or_put="call",
    direction="long",
    amount=1.0,
    time_to_exp=0.0,
...
def make_payoff_cfg(
    x_pts, strike_pct: float, payoff_dict, payoff_fcn=black_scholes_payoff
):
    """
    ...
    if "type" in payoff_dict and "dir" in payoff_dict and "amt" in payoff_dict:
        builder.add_option_leg(
            call_or_put=payoff_dict["type"],
            direction=payoff_dict["dir"],
            amount=payoff_dict["amt"],
            time_to_exp=payoff_dict["t"],
            sigma=payoff_dict["sigma"],
            r=payoff_dict["r"],
            q=payoff_dict["q"],
            strike=strike_pct,
        )
    ...
    if "und_price" in payoff_dict and "und_amt" in payoff_dict:
        builder.set_underlying_payoff_leg(
            underlying_price=payoff_dict["und_price"],
            underlying_amount=payoff_dict["und_amt"],
        )

    if "other_legs" in payoff_dict:
        for leg in payoff_dict["other_legs"]:
            builder.add_option_leg(
                call_or_put=leg["type"],
                direction=leg["dir"],
                amount=leg["amt"],
                strike=leg["strike"],
                time_to_exp=leg["t"],
...
def build_generator_config_from_files(
    input_file: str,
    training_history_file: str,
    payoff_dict=None,
    lower_bounds_fcns=None,
    upper_bounds_fcns=None,
):
    """
    ...
    training_config = (
        ...
        .set_input_csv_filename(input_file)
        .set_training_history_filename(training_history_file)
    )
    ...
    if payoff_dict is not None:
        ...
        if "dir" not in payoff_dict or payoff_dict["dir"] == "long":
            upper_bounds.append(lambda a: -1e-10)
            lower_bounds.append(lambda a: -1.0)
        else:
            upper_bounds.append(lambda a: 1.0)
            lower_bounds.append(lambda a: 1e-10)
    else:
        upper_bounds.append(lambda a: 1.0)
        lower_bounds.append(lambda a: 1e-10)

    if lower_bounds_fcns is not None:
        [lower_bounds.append(func) for func in lower_bounds_fcns]

    if upper_bounds_fcns is not None:
        [upper_bounds.append(func) for func in upper_bounds_fcns]

    bounds_config = ConstraintsConfig(upper_bounds, lower_bounds)

    return GeneratorConfig(
        train_config=training_config.build_config(),
        fit_config=FitConfigBuilder().build_config(),
...
def build_generator_config(
    ...
    return GeneratorConfig(
        train_config=training_config.from_config(configs, price_history),
        fit_config=FitConfigBuilder().build_config(),
        bounds_config=bounds_config,
    )
...
def training_output_to_csv(dfs: List[pd.DataFrame]):
    ...
    return (
        pd.DataFrame(rows)
        .drop_duplicates(
            subset=["Ticker", "Label", "CurrentPrice", "StartDate", "EndDate"]
        )
        .reset_index(drop=True)
    )


def training_output_to_convolution_config(
    dfs, min_x=-5.0, max_x=5.0, log_only=False, pdf_pts=20001, dist=skewed_t
):
    """
    Takes the DataFrames output from the training process and repackages the output
    ...
    for df in dfs:
        ...
        conv_cfgs.append(
            ConvolutionConfigBuilder()
            .set_num_times_to_convolve(df["Expiration"].max())
            .set_min_x(min_x)
            .set_max_x(max_x)
            .set_log_only(log_only)
            .set_points_in_pdf(pdf_pts)
            .set_distribution(dist(*args, loc=params[0], scale=params[1]).pdf)
            .set_distribution_params(params)
            .build_config()
        )
...
def training_output_to_payoff_config(
    dfs: List[pd.DataFrame], conv_configs: List[ConvolutionConfig], payoff_dict=None
):
    """
    Takes the DataFrames output from the training process and repackages the output
    ...
    return [
        [
            make_payoff_cfg(conv_cfg.x, strike_pct, payoff_dict)
            for strike_pct in df["StrikePct"].values
        ]
        for df, conv_cfg in zip(dfs, conv_configs)
    ]


def add_pdf_csv_columns(
    exp_days: np.ndarray,
...
def add_curve_csv_rows(
    df: pd.DataFrame, outputs, dist_params, bet_fractions=np.linspace(0.0, 0.9999, 50)
):
    """

...
def transform_probability_bin(
    x1: float, y1: float, x2: float, y2: float, x1_t: float, y1_t: float, x2_t: float
):
    """
...
def transform_pdf_rough(
    starting_domain_pts: np.ndarray,
    ...
    """
    if len(starting_domain_pts) != len(starting_domain_pdf) or len(
        starting_domain_pts
    ) != len(transformed_domain_pts):
        raise ValueError(
...
def sum_squared_error_bin_areas(
    transformed_pdf: np.ndarray,
    starting_domain_pts: np.ndarray,
    starting_domain_pdf: np.ndarray,
    transformed_domain_pts: np.ndarray,
):
...
def transform_pdf_using_optimize(
    starting_domain_pts: np.ndarray,
    starting_domain_pdf: np.ndarray,
    transformed_domain_pts: np.ndarray,
    bounds="default",
    ...
    # NOTE: 4 Transform the PDF from log returns to prices

    # Calculates an approximate initial value of what the transformed pdf will be
    transformed_guess = transform_pdf_rough(
        starting_domain_pts, starting_domain_pdf, transformed_domain_pts
    )
    ...
    if bounds is None:
        transformed_pdf = minimize(
            sum_squared_error_bin_areas,
            transformed_guess,
            args=(starting_domain_pts, starting_domain_pdf, transformed_domain_pts),
            tol=tol,
        )
    else:

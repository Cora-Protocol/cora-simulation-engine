...
def run_study(
    ...
    save_folder: str = "studies/008-v2-blackswan/results",
    base_seed: int = 95739,
):
    ...
    for hh, start_time in enumerate(WORST_MONTHS):
        ...
        for ii, (
            ...
            print(
                f"{ii + 1}/{total_study_variations} RUNNING studies for {start_time_str=} {volatility_factor=}, {borrower_demand=}, {loan_start=}, {loan_duration=}"
            )
            subfolder_name = f"start_{start_time_str}_vol_{float_to_str(volatility_factor)}_demand_{float_to_str(borrower_demand)}_start_{loan_start}_duration_{loan_duration}"
            ...
            for jj, (max_ltv, fee_model) in enumerate(
                ...
                print(
                    f"{jj+1}/{num_studies}: Running study for {max_ltv=}, {fee_model=} with {num_seeds} start times"
                )
                strategy_name = f"corabs_{start_time_str}_maxltv_{float_to_str(max_ltv)}_feemodel_{fee_model}_n_{num_seeds}_s_{seed}"

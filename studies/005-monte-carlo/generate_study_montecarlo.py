...
CONFIG = SimulationResultConfig(
    results_folder="studies/005-monte-carlo/results",
...
class PerformancePoint(NamedTuple):
...
def iterate_performance_points(
...
def generate_study(
    ...
    loan_start_type_options: List[DistOptions] = LOAN_START_OPTIONS,
    loan_duration_type_options: List[DistOptions] = LOAN_DURATION_OPTIONS,
    start_time: datetime = BACKTESTING_START_TIME,
    num_randoms: int = 100,
    volatility_factor: float = 1.0,
    random_seed: int = 0,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    total_sim_time = GENESIS_PERIOD + RUNNING_PERIOD + timedelta(hours=1)
    environment = BrownianCoraEnvironment("ETH", volatility_factor=volatility_factor)
    ...
    df_input = []
    timeseries_dfs = []
    for backtesting_point in iterate_performance_points(
        ...
        for j, random_seed in enumerate(range(random_seed, random_seed + num_randoms)):
            ...
            step_metrics["loan_duration_type"] = backtesting_point.loan_duration_type
...
def run_and_save_study(
    ...
    csv_filename = f"montecarlo_{fee_model}_ltv{float_to_name(max_ltv)}_up{float_to_name(utilization_param)}_x{num_randoms}_vf{float_to_name(volatility_factor)}.csv"
...
def run_and_save_study_array(
    ...
    if compress_to_zip:
        zip_filename = f"00_montecarlo_{fee_model}_x{num_randoms}.zip"

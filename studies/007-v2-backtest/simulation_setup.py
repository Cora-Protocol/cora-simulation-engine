...
def run_study(
    ...
    for ii, (
        ...
        for jj, (max_ltv, fee_model) in enumerate(
            ...
            print(
                f"{jj+1}/{num_studies}: Running study for {max_ltv=}, {fee_model=} with {num_start_times} start times"
            )
            strategy_name = f"corabt_maxltv_{float_to_str(max_ltv)}_feemodel_{fee_model}_n_{num_start_times}_s_{seed}"

...
def run_study(
    ...
    save_folder: str = "studies/007-v2-backtest/results",
    base_seed: int = 95739,
):
    ...
    for ii, (borrower_demand, loan_start, loan_duration,) in enumerate(
        product(
        ...
        print(
            f"{ii + 1}/{total_study_variations} RUNNING studies for {borrower_demand=}, {loan_start=}, {loan_duration=}"
        )
        subfolder_name = f"demand_{float_to_str(borrower_demand)}_start_{loan_start}_duration_{loan_duration}"
        subfolder = Path(save_folder) / subfolder_name
        subfolder.mkdir(parents=True, exist_ok=True)

        environment = HistoricalCoraEnvironment(asset)
        ...
        for jj, (max_ltv, fee_model) in enumerate(
            ...
            print(
                f"{jj+1}/{num_studies}: Running study for {max_ltv=}, {fee_model=} with {len(start_times)} start times"
            )
            strategy_name = f"corabt_{asset}_maxltv_{float_to_str(max_ltv)}_feemodel_{fee_model}_n_{len(start_times)}_s_{seed}"
            ...
            rows = []
            for kk, start_time in enumerate(start_times):
